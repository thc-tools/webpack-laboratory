...
module.exports = (processEnv, argv) => {
    ...
    config.plugins.push(new CleanWebpackPlugin(['js', 'misc', 'chunks', 'img', 'fonts', 'css', 'html'], { allowExternal: true, root: config.output.path }));
    ...
    if (isHotReload) {
        ...
        config.plugins.push(new MiniCssExtractPlugin({ filename: `css/[name]_${env.npm_package_version}.bundle.css`, chunkFilename: 'css/[name]_[hash].css' }))
    }
